...
sp500['price_FD10'] = sp500['price'].shift(-10).fillna(sp500['price'].iloc[-1])

# Analyze and visualize price distribution
print("Price column statistics before filtering:")
print(sp500['price'].describe())

sns.histplot(sp500['price'], bins=50, kde=True)
...
price_lower_threshold = 0.05
price_upper_threshold = 2.0
sp500 = sp500[(sp500['price'] > price_lower_threshold) & (sp500['price'] < price_upper_threshold)]
print(f"Dataset size after filtering: {len(sp500)} rows remaining.")

if len(sp500) < 300:
    print(f"Warning: Filtered dataset is small ({len(sp500)} rows). Inspect manually.")
    sp500.to_csv("filtered_dataset_inspection.csv", index=False)

# Cap returns
return_lower_threshold = -0.2
return_upper_threshold = 0.2
sp500[['ret_D10', 'ret_FD10']] = sp500[['ret_D10', 'ret_FD10']].clip(
    lower=return_lower_threshold, upper=return_upper_threshold
)

# Visualize price distribution after filtering
sns.histplot(sp500['price'], bins=50, kde=True)
...
sp500.to_csv("model_processed_data_GAN-script.csv", index=False)
print("Data filtering and preprocessing complete.")
print(f"Dataset size after filtering: {len(sp500)} rows remaining.")
if len(sp500) < 300:  # Or another threshold based on your requirements
    print(f"Warning: Filtered dataset is small ({len(sp500)} rows). Adjust filtering thresholds.")
...
def createTrainTest(stock, features, testSize=252):
    ...
    if totalRecords <= testSize:
        raise ValueError("Insufficient data to split into train and test sets.")

    testSize = min(testSize, totalRecords // 2)  # Adjust testSize if dataset is small
    trainSize = totalRecords - testSize
    train = np.arange(0, trainSize)
    test = np.arange(trainSize, totalRecords)
...
def calcPnL(model, stock, X_test, y_test, threshold=0.5, testSize=252, stock_name=None):
    ...
    # Predicted probabilities and bet sizes
    analysis['probUP'] = model.predict(X_test)
    analysis['betSize'] = np.clip(analysis['probUP'] - 0.5, -0.05, 0.05)  # Stricter cap

    # Daily profit and loss
    analysis['dailyP&L'] = analysis['ret_FD10'] * analysis['betSize']

    # Cumulative return using a logarithmic approach
    analysis['logDailyP&L'] = np.log1p(analysis['dailyP&L'])  # Logarithmic stability
    analysis['cumulativeReturn'] = np.expm1(analysis['logDailyP&L'].cumsum())  # Reverse log(1 + x)

    profitLSTM = analysis['cumulativeReturn'].iloc[-1] * 100
    profitBuyHold = (analysis.loc[analysis.index[-1], 'buyAndHold']) * 100

    # Plot cumulative returns
    plt.figure(figsize=(10, 6))
    plt.plot(analysis['cumulativeReturn'], label='LSTM Attention, Return = {:.2f} %'.format(profitLSTM))
    ...
    return profitLSTM, profitBuyHold
...
modelSP500.fit(X_train_LSTM, y_train_LSTM, validation_data=(X_test_LSTM, y_test_LSTM),
               epochs=100, batch_size=28, verbose=2, callbacks=[early_stopping])
